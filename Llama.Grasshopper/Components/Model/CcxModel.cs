...
using Llama.Gh.Conversion;
using Llama.Gh.Definitions;
...
namespace Llama.Gh.Components
{
    ...
    {
        public CcxModel()
            : base("CcxModel", "CcxModel", "Assemble a CcxModel from element models, supports, and steps.", "Llama", "Model")
        {
            Message = Name + "\nLlama";
        }
        ...
        {
            pManager.AddTextParameter("Name", "Name", "Model name.", GH_ParamAccess.item, "LlamaModel");
        ...
        {
            var resolvedLoads = new List<Llama.Core.Model.Loads.NodalLoad>(step.NodalLoads.Count);
        ...
        private List<StructuralModel> BuildSourceModels(IEnumerable<object> modelObjects, IList<SectionBase> inferredSections, double tolerance)
        {
            ...
            foreach (var obj in modelObjects)
            {
                ...
                {
                    var converted = RhinoHexMeshToLlamaConverter.CreateModelFromHexMeshes(
                        hexMesh.Meshes,
                        tolerance,
                ...
                {
                    var converted = RhinoTetraMeshToLlamaConverter.CreateModelFromTetraMeshes(
                        tetraMesh.Meshes,
                        tolerance,
