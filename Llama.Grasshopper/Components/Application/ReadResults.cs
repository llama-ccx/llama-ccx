    ...
    public class ReadResultsComponent : GH_SwitcherComponent
    {
        ...
        public ReadResultsComponent()
            : base("ReadResults", "ReadDat",
                "Read nodal results (U, RF force/moment) and element stresses from a CalculiX .dat file. Request RF via *NODE PRINT (OutputRequest).", "Llama", "Application")
        {
            Message = Name + "\nLlama";
        }
        ...
        protected override void RegisterEvaluationUnits(EvaluationUnitManager mngr)
        {
            ...
            unit.RegisterInputParam(new Param_String { Optional = true }, "Set Name", "S", "Optional CalculiX set name (NodeSet or ElementSet) to filter results. Leave empty to return all results.", GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA, EvaluationUnit unit)
        {
            ...
            // ── Cache: only re-parse when model/set changes ──
            ...
            {
                // Nodal outputs
                if (_cachedDisplacements != null && _cachedDisplacements.Count > 0)
                {
                    DA.SetDataList(1, MapNodePoints(_cachedDisplacements, nodeMap));
                    DA.SetDataList(2, MapDisplacementVectors(_cachedDisplacements));
                }
                else
                {
                ...
                {
                    DA.SetDataList(3, MapReactionNodePoints(_cachedReactions, nodeMap));
                    DA.SetDataList(4, MapReactionForces(_cachedReactions));
                    DA.SetDataList(5, MapReactionMoments(_cachedReactions));
                }
                else
                {
                ...
                if (_cachedStresses != null && _cachedStresses.Count > 0)
                {
                    ...
                    foreach (var stress in _cachedStresses)
                    {
